...
create() {
   ...
   this.timerBackground.setDepth(9);         // Establecer la profundidad para asegurarse de que se dibuje encima de otros elementos
    ...
    nombreA=this.add.text(220,20, " 0 ", {font: "30px Arial Black"});      // AJUSTAR LETRA, TAMAÑO, ETC
    ...
    nombreB=this.add.text(945,20, " 0 ", {font: "30px Arial Black"});      // AJUSTAR LETRA, TAMAÑO, ETC
...
async nuevaPartida(mapaElegido) {
    try{
        ...
        this.asignarPersonajes(this.gameID);
...
async asignarPersonajes(gameId){
    try {
        const response = await fetch(`http://localhost:8080/api/games/players/game/${gameId}`);
        this.mostrarErrorConexionServidor(response.status);

        const listaUsuarios = response.json();
        this.nombreA = listaUsuarios[0].username;
        this.nombreB =  listaUsuarios[1].username;

    } catch(error){
        throw new Exception("Error en el servicio de recuperación de jugadores por partida: " + error);
    }
}
...
mostrarErrorConexionServidor(status){
    ...
    if(httpErrors.includes(status)) alert("Se ha perdido la conexión con el servidor");
}
